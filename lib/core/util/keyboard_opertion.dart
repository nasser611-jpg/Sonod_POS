...
class keyboardOperions {
  ...
  static void onTapNumber(BuildContext context, List<Product> products,
      String value, List<FormattedProduct> formattedProducts) {
    ...
    if (featchBlocById(context).clickedItem) {
      ...
      if (featchBlocById(context).proudctSelectedId ==
          formattedProducts![featchBlocById(context).index].productId) {
        bool removed = false;
        ...
        while (!removed) {
          ...
          for (int i = 0; i < products.length; i++) {
            ...
            if (element.productId ==
                featchBlocById(context).proudctSelectedId) {
              if (products
                      .where((p) => p.productId == element.productId)
                      .length >
                  1) {
                removed = false;
                products.remove(element);
