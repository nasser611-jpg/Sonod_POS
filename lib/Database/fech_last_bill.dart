...
Future<int> lastBillId() async {
  DbHelper dbO = DbHelper();
    ...
    final List<Map<String, dynamic>> result = await db.rawQuery('SELECT MAX(bill_id) as max_id FROM bills');
    final int lastId = result.isNotEmpty ? result.first['max_id'] as int : 0;
    return lastId;
  }
  ...
  DbHelper dbO = DbHelper();
     Database db = await dbO.createDatabase();
    final List<Map<String, dynamic>> result = await db.rawQuery('SELECT bill_date FROM bills ORDER BY bill_id DESC LIMIT 1');
